...
def _lesson_log_fundamentals(progress):
    ...
    lesson_block(
        ...
        "events, errors, access attempts, and state changes that let security "
        "analysts reconstruct what happened — and when — during a breach or "
        "operational incident.  Without logs, incident response is little more "
        "than guesswork."
    )
    ...
    lesson_block(
        ...
        "They record kernel messages, service starts/stops, cron jobs, and "
        "firewall events."
    )
    ...
    lesson_block(
        ...
        "succeeded or failed, and from which IP address. On Linux: "
        "/var/log/auth.log (Debian/Ubuntu) or /var/log/secure (RHEL/CentOS). "
        "These are the first place to look when investigating brute-force "
    ...
    lesson_block(
        "3. WEB SERVER ACCESS LOGS — Apache and Nginx write a line for every HTTP "
        "request.  The Common Log Format includes: client IP, timestamp, request "
        "method/URI, HTTP status, and response size.  These reveal scanning, "
        "injection attempts, and data exfiltration."
    )
    ...
    lesson_block(
        ...
        "at multiple severity levels: DEBUG, INFO, WARN, ERROR, CRITICAL.  They "
        "capture business-logic events such as failed logins, rate-limit hits, "
        "database errors, and deployment activities."
    )
    ...
    lesson_block(
        ...
        "command.  On Windows, the Event Viewer (eventvwr.msc) provides access "
        "to Security, Application, and System event logs."
    )
    ...
    lesson_block(
        "SYSLOG FORMAT (RFC 5424): <priority>version timestamp hostname app-name "
        "procid msgid structured-data msg.  In practice you usually see the "
        "BSD-style variant: 'Month Day HH:MM:SS hostname process[PID]: message'."
    )
    ...
    lesson_block(
        "JSON-STRUCTURED LOGS are increasingly popular in modern applications "
        "because they are easy to parse with tools like jq, Python, or log "
        "aggregation systems (ELK, Splunk, Datadog)."
    )
    ...
    why_it_matters(
        ...
        "logging with retention periods of 90 days to one year. Without adequate "
        "logging, your organization cannot detect breaches, satisfy auditors, or "
...
def _lesson_parsing_logs(progress):
    ...
    lesson_block(
        "Manually reading log files is impractical when a busy server generates "
        "millions of lines per day.  Security analysts write parsers — small "
        "programs that extract structured data from raw log text so it can be "
        "filtered, counted, and correlated.  Python, with its built-in 're' "
        "(regular expression) module, is an ideal tool for this."
    )
    ...
    lesson_block(
        "Below is a complete Python script that reads auth.log data, counts "
        "failed SSH login attempts per IP address, and flags any IP that exceeds "
        "a configurable threshold.  Study the code carefully — it is a pattern "
        "you can adapt to many log-analysis tasks."
    )
    ...
    lesson_block(
        ...
        "Combined Log Format to find suspicious patterns like repeated 401s, "
        "path-traversal attempts, and common scanner probes."
    )
    ...
    why_it_matters(
        ...
        "can process them in seconds and surface only the events that need human "
        "attention. This is the foundation of SIEM (Security Information and "
        "Event Management) systems like Splunk, Elastic SIEM, and Microsoft "
        "Sentinel."
    )
    ...
    lesson_block(
        "Using the SAMPLE_ACCESS_LOG data above, write a mental model (or "
        "actual code) that counts how many HTTP 401 responses each IP received. "
        "Which IP has the most 401s, and how many?"
    )
    hint_text(
...
def _lesson_alerting_scripts(progress):
    ...
    lesson_block(
        ...
        "file in near-real-time and fires alerts when suspicious patterns exceed "
    ...
    lesson_block(
        ...
        "false positives) while still catching real threats.  Here are sensible "
        "starting points:"
    )
    ...
    lesson_block(
        ...
        "PagerDuty, and writing to a SIEM.  Here is how you would send an email "
        "alert from Python using the built-in smtplib:"
    )
    ...
    lesson_block(
        "For Slack webhooks, you would POST a JSON payload to a webhook URL.  "
        "This is often preferred for DevOps teams because it puts alerts where "
        "the team is already communicating:"
    )
    ...
    lesson_block(
        "This script demonstrates how to watch a log file for new entries and "
        "check them against alert rules.  It uses a 'tail -f' style approach, "
        "reading from the last known position in the file.  In production you "
        "would run this as a systemd service or in a container."
    )
    ...
    code_block(
        ...
        '        # send_alert_email("Brute Force Detected", msg, "admin@co.com")\n'
    ...
    why_it_matters(
        ...
        "that to minutes or seconds, drastically limiting the damage an attacker "
        "can do."
    )
...
def _lesson_automating_checks(progress):
    ...
    lesson_block(
        "Continuous security is not a one-time effort.  Mature organizations run "
        "automated daily, weekly, and monthly security checks that surface "
        "problems before attackers find them.  In this lesson we build a daily "
        "security check script, learn how to schedule it with cron, format "
        "reports, and cover the fundamentals of incident response."
    )
    ...
    lesson_block(
        "A daily security check script typically performs several automated "
        "inspections and compiles the results into a report.  Here is a "
    ...
    code_block(
        ...
        '    "/etc/sudoers",\n'
        ']\n'
        'KNOWN_HASHES_FILE = Path("/etc/security/known_hashes.json")\n'
        '\n'
        ...
        '                h.update(f.read())\n'
        '            digest = h.hexdigest()\n'
        '            # Compare against known_hashes (omitted for brevity)\n'
        '            self.add_finding("INFO", "INTEGRITY",\n'
        ...
        '            if files:\n'
        '                self.add_finding("CRITICAL", "PERMISSIONS",\n'
        '                    f"{len(files)} world-writable files in /etc")\n'
    ...
    lesson_block(
        ...
        "debug failures.  (3) Set the PATH environment variable at the top of "
        "the crontab.  (4) On modern systems, consider using systemd timers "
        "instead, which offer better logging and dependency management."
    )
    ...
    lesson_block(
        ...
        "rating, a category, a clear description of the finding, and a "
        "recommended action.  Here is a sample output from the script above:"
    )
    ...
    lesson_block(
        ...
        "defined roles and responsibilities, communication templates, contact "
        "lists for legal/PR/management, pre-authorized actions (e.g., 'the "
        "on-call engineer may block IPs without manager approval'), and "
        "prepared forensic tools."
    )
    ...
    lesson_block(
        ...
        "scope (which systems, which data), assess severity, and document "
        "everything with timestamps."
    )

    lesson_block(
        "PHASE 3: CONTAINMENT, ERADICATION & RECOVERY\n"
        "CONTAIN: Stop the bleeding.  Isolate affected systems from the network, "
        "block attacker IPs, disable compromised accounts, revoke tokens.  "
        "ERADICATE: Remove the threat.  Delete malware, patch the vulnerability, "
        "close the backdoor.  "
        "RECOVER: Restore normal operations.  Bring systems back online from "
        "clean backups, reset credentials, verify integrity."
    )

    lesson_block(
        "PHASE 4: LESSONS LEARNED (Post-Incident Review)\n"
        "Within 1-2 weeks of the incident, hold a blameless post-mortem. "
        "Document: What happened?  When was it detected?  What went well? "
        "What could be improved?  What specific actions will prevent recurrence? "
        "Update your runbooks, monitoring rules, and training based on findings."
    )
    ...
    code_block(
        ...
        '    )\n'
        '    # Using subprocess with explicit arguments (no shell=True!)\n'
        '    result = subprocess.run(\n'
    ...
    why_it_matters(
        ...
        "opened port 3306 to the internet or made /etc/shadow world-readable "
